...
function DashBoard(data){
    let roomData = data.roomData?data.roomData:[];
    let eventDetail = data.eventDetail? data.eventDetail:[];
    let updatedEventDetail = data.updatedEventDetail;
    var events_d = []
    var id = 0
        ...
        <div id="event_content">
        {
            eventDetail.map(row => {
                    var re_obj = (
                        ...
                        <div class="textLeft p005 eventDetailContent " dangerouslySetInnerHTML={{ __html: row[1] }} />
                        </div>
                    )
                    return(re_obj)
                }
