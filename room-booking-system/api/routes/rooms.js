...
const dateAEST = date => {
  //return momentTimezone.tz(date, "Asia/Taipei");
  return momentTimezone.tz(date, "Asia/Taipei");
}
...
router.put('/rooms/:id', requireJWT, (req, res) => {
  ...
  // If the booking is a recurring booking
    ...
    let bookingDateTracker = momentTimezone(firstBooking.bookingStart).tz("Asia/Taipei")
    ...
    let lastBookingDate = momentTimezone(firstBooking.recurring[0]).tz("Asia/Taipei")
    ...
    for (let i = 0; i < bookingsInRange; i++) {
      ...
      if (1) {
        ...
        let firstBookingEndDate = momentTimezone(firstBooking.bookingEnd).tz("Asia/Taipei")
